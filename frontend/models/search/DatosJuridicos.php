<?php

namespace frontend\models\search;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use frontend\models\DatosJuridicos as DatosJuridicosModel;

/**
 * DatosJuridicos represents the model behind the search form of `app\models\DatosJuridicos`.
 */
class DatosJuridicos extends DatosJuridicosModel
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'estado_oficina'], 'integer'],
            [['oficina_registro', 'numero', 'folio', 'tomo', 'fecha', 'rif', 'nit'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = DatosJuridicosModel::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'estado_oficina' => $this->estado_oficina,
            'fecha' => $this->fecha,
        ]);

        $query->andFilterWhere(['ilike', 'oficina_registro', $this->oficina_registro])
            ->andFilterWhere(['ilike', 'numero', $this->numero])
            ->andFilterWhere(['ilike', 'folio', $this->folio])
            ->andFilterWhere(['ilike', 'tomo', $this->tomo])
            ->andFilterWhere(['ilike', 'rif', $this->rif])
            ->andFilterWhere(['ilike', 'nit', $this->nit]);

        return $dataProvider;
    }
}
